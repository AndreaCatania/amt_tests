...
use crate::game_state::{Motion, Tool};
...
impl<'s> System<'s> for ToolSystem {
    ...
    fn run(&mut self, (time, input_events, tools, mut transforms): Self::SystemData) {
        ...
        for (transf, _) in (&mut transforms, &tools).join() {
            ...
            delta_transf.append_rotation_y_axis(
                m_motion_x.to_radians() * MOUSE_SENSITIVITY * time.delta_seconds(),
            );
            delta_transf.append_rotation_z_axis(
                m_motion_y.to_radians() * MOUSE_SENSITIVITY * time.delta_seconds() * -1.0,
            );
